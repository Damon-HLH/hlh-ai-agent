...
import com.hlh.hlhaiagent.model.LoveReport;
...
public class DatabaseChatMemory implements ChatMemory {
    ...
    private final LoveReportMapper loveReportMapper;

    /**
     * 构造函数，初始化 LoveReportMapper。
     *
     * @param loveReportMapper 恋爱报告数据库操作 Mapper
     */
    public DatabaseChatMemory(LoveReportMapper loveReportMapper) {
        this.loveReportMapper = loveReportMapper;
    }
...
//        // 计算需要跳过的消息数量
//        int skipCount = Math.max(0, allMessages.size());
        return allMessages.stream().toList(); // 返回最后 N 条消息
    }
    ...
    public void clear(String conversationId) {
        try {
            // 构建查询条件
            LambdaQueryWrapper<LoveReport> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.eq(LoveReport::getChatId, getSafeConversationId(conversationId));

            // 删除对应的记录
            int deletedRows = loveReportMapper.delete(queryWrapper);
            if (deletedRows > 0) {
    ...
    private List<Message> getOrCreateConversation(String conversationId) {
        ...
        try {
            // 构建查询条件
            LambdaQueryWrapper<LoveReport> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.eq(LoveReport::getChatId, safeConversationId);

            // 查询数据库
            LoveReport loveReport = loveReportMapper.selectOne(queryWrapper);
    ...
    private void saveConversation(String conversationId, List<Message> messages) {
        ...
        try {
            ...
            // 查询数据库中是否已存在该对话的记录
            LoveReport existingReport = loveReportMapper.selectOne(queryWrapper);

            if (existingReport != null) {
                // 更新现有记录的 messages 字段
                existingReport.setMessages(messagesJson);
                int updatedRows = loveReportMapper.updateById(existingReport);
                if (updatedRows > 0) {
                ...
                // 创建新记录
                LoveReport newReport = LoveReport.builder()
                        .chatId(safeConversationId)
                        // 存储序列化后的完整消息列表
                        .messages(messagesJson)
                        .build();
                int insertedRows = loveReportMapper.insert(newReport);
                if (insertedRows > 0) {
