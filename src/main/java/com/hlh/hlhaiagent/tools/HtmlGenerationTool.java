...
public class HtmlGenerationTool {
    ...
    private String buildHtmlDocument(String title, String bodyContent) {
        ...
        if (title != null && !title.trim().isEmpty()) {
            ...
            try {
                // 先处理 Markdown 格式的 h1 标题（# Title）
                processedContent = removeMarkdownH1(processedContent, plainTitle);
        ...
        // 将 Markdown 内容转换为 HTML（支持图片、列表、加粗等语法）
     ...
     * 使用 commonmark-java 库解析 Markdown 语法（包括表格扩展），
     * 使 agent 传入的 Markdown 内容（图片 ![](url)、列表、加粗等）能正确渲染为 HTML
     */
    private String convertMarkdownToHtml(String markdownContent) {
        ...
        try {
            ...
            log.warn("Markdown转换为HTML时出错，将使用原始内容: {}", e.getMessage());
            return markdownContent;
        }
    }

    /**
     * 移除 Markdown 格式的 h1 标题（# Title），防止与自动生成的标题重复
     */
    private String removeMarkdownH1(String content, String title) {
        if (content == null || title == null) return content;

        // 匹配行首的 # 加空格（Markdown h1 语法）
        Pattern mdH1Pattern = Pattern.compile("^#\\s+(.+)$", Pattern.MULTILINE);
        Matcher matcher = mdH1Pattern.matcher(content);
        ...
        while (matcher.find()) {
            ...
            if (h1Text.equalsIgnoreCase(cleanedTitle) || similarText(h1Text, cleanedTitle)) {
                // 匹配到重复标题，移除该行
